...
// Section data (with Chinese numerals for first-version style)
const sections = [
...
// ========== FIRST-VERSION COMPONENTS (used in wideScreen mode) ==========
...
function ChapterOpener({ num, title, id }: { num: string; title: string; id: string }) {
  return (
    <div id={id} className="pt-24 pb-10">
      ...
      <h2 className="font-serif text-3xl md:text-[2.5rem] font-bold text-[#1A1A1A] leading-tight tracking-tight">
        {title}
      </h2>
    </div>
  );
...
function V1DefinitionCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="my-10 relative pl-6 border-l-[3px] border-[#1E40AF]/50">
      <div className="absolute -left-[11px] top-0 w-[19px] h-[19px] rounded-full bg-white border-[3px] border-[#1E40AF]/50" />
      <div className="text-xs font-bold text-[#1E40AF] uppercase tracking-[0.15em] mb-2">名词解释</div>
      <div className="font-serif text-lg font-semibold text-[#1A1A1A] mb-2">{title}</div>
      <div className="text-[0.95rem] leading-[1.8] text-[#374151]">{children}</div>
    </div>
  );
}
...
function PullQuote({ children }: { children: React.ReactNode }) {
  return (
    <div className="my-14 py-8 -mx-4 md:-mx-8 px-8 md:px-12 border-y border-[#1E40AF]/15 bg-gradient-to-r from-[#1E40AF]/[0.02] to-transparent">
      <p className="font-serif text-xl md:text-2xl leading-relaxed text-[#1A1A1A]/90 font-medium italic">
        {children}
      </p>
    </div>
  );
...
function Highlight({ children }: { children: React.ReactNode }) {
  return (
    <blockquote className="my-10 py-5 px-6 border-l-[3px] border-[#1E40AF] bg-[#1E40AF]/[0.03] rounded-r-md">
      <p className="font-serif text-lg leading-[1.8] text-[#1A1A1A]/85 italic">{children}</p>
    </blockquote>
  );
}

function V1CaseCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="my-10 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg px-6 py-6">
      ...
      <div className="font-serif text-base font-semibold text-[#1A1A1A] mb-3">{title}</div>
      <div className="text-[0.95rem] leading-[1.8] text-[#374151]">{children}</div>
    </div>
  );
}

function V1PromptCard({ children }: { children: React.ReactNode }) {
  return (
    <div className="my-10 bg-[#0F172A] rounded-lg px-6 py-5 shadow-lg">
      ...
      <div className="font-mono text-sm leading-[1.8] text-white/85">{children}</div>
    </div>
  );
}

function V1FrameworkCard({ items }: { items: { label: string; desc: string; active?: boolean }[] }) {
  return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3">
        {items.map((item, i) => (
          <div key={i} className={`p-5 text-center ${i < items.length - 1 ? 'border-b md:border-b-0 md:border-r border-[#E2E8F0]' : ''} ${item.active ? 'bg-[#1E40AF]/[0.04]' : ''}`}>
            <div className={`text-xs font-bold uppercase tracking-[0.12em] mb-2 ${item.active ? 'text-[#1E40AF]' : 'text-[#666]'}`}>{item.label}</div>
            <div className="font-serif text-base font-medium text-[#1A1A1A]">{item.desc}</div>
          </div>
        ))}
      </div>
    </div>
...
// ========== CURRENT-VERSION COMPONENTS (used in non-wideScreen modes) ==========

function DefinitionCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="my-8 py-5 px-6 bg-[#f7f7f8] border border-[#ececed] rounded-md">
      <div className="text-xs font-semibold text-[#666] uppercase tracking-wider mb-2">名词解释</div>
      <div className="text-base font-bold text-[#0d0d0d] mb-2">{title}</div>
      <div className="text-[15px] leading-[1.75] text-[#374151]">{children}</div>
    </div>
  );
}
...
function CaseCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="my-8 py-5 px-6 bg-[#f7f7f8] border border-[#ececed] rounded-md">
      <div className="text-xs font-semibold text-[#666] uppercase tracking-wider mb-2">课堂案例</div>
      <div className="text-base font-bold text-[#0d0d0d] mb-2">{title}</div>
      <div className="text-[15px] leading-[1.75] text-[#374151]">{children}</div>
    </div>
  );
...
function PromptCard({ children }: { children: React.ReactNode }) {
  return (
    <div className="my-8 bg-[#0d0d0d] rounded-md px-6 py-5">
      <div className="text-xs text-white/40 font-mono mb-3 uppercase tracking-wider">Prompt 示例</div>
      <div className="font-mono text-sm leading-[1.8] text-white/85">{children}</div>
    </div>
  );
}
...
function FrameworkCard({ items }: { items: { label: string; desc: string; active?: boolean }[] }) {
  return (
    <div className="my-8 border border-[#ececed] rounded-md overflow-hidden">
      <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-[#ececed]">
        {items.map((item, i) => (
          <div key={i} className={`p-5 text-center ${item.active ? 'bg-[#f7f7f8]' : ''}`}>
            <div className={`text-xs font-semibold uppercase tracking-wider mb-2 ${item.active ? 'text-[#0d0d0d]' : 'text-[#999]'}`}>{item.label}</div>
            <div className="text-[15px] font-medium text-[#0d0d0d]">{item.desc}</div>
          </div>
        ))}
      </div>
    </div>
...
export default function Home() {
  ...
  if (wideScreen) {
    return (
      <div className="min-h-screen bg-white">
        ...
        {/* Hero section - first version style */}
          ...
          <div className="relative max-w-[720px] mx-auto px-6">
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-12">
                ...
                <span className="text-sm text-[#1A1A1A]/50 font-medium tracking-wide">面向小学教师与大众的 AI 智能体认知开场稿</span>
              </div>

              {/* Title */}
              <h1 className="font-serif text-5xl md:text-7xl font-bold text-[#1A1A1A] leading-[1.1] tracking-tight mb-8">
                人工智能如水
              </h1>

              {/* Subtitle */}
              <p className="text-lg md:text-xl text-[#1A1A1A]/50 leading-relaxed mb-8 max-w-[520px]">
                修订版 v2 · 增补代码智能体拐点与 Harness Engineering
              </p>

              {/* Meta */}
              <div className="flex items-center gap-4 text-sm text-[#1A1A1A]/35">
            ...
            <motion.div
              ...
              className="mt-16 pt-10 border-t border-[#1A1A1A]/8"
            >
              <p className="font-serif text-2xl md:text-3xl leading-[1.6] text-[#1A1A1A]/75 italic tracking-tight">
        ...
        {/* Exit widescreen button - floating */}
        <div className="fixed top-4 right-6 z-50">
          <button
            onClick={() => setWideScreen(false)}
            className="px-4 py-2 text-sm bg-white/90 backdrop-blur-sm border border-[#E2E8F0] rounded-lg shadow-sm text-[#1E40AF] font-medium hover:bg-white transition-colors"
          >
            退出大屏
          </button>
        </div>

        {/* Main content area - first version layout */}
          ...
          <div className="flex gap-16">
            ...
            <aside className={`hidden lg:block w-[180px] shrink-0 transition-all duration-500 ${showNav ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
              <nav className="sticky top-12 pt-8">
                <div className="text-[10px] font-bold text-[#1A1A1A]/30 uppercase tracking-[0.2em] mb-5">目录</div>
                      ...
                      <button
                        ...
                        className={`group text-left w-full text-[13px] py-2 px-3 rounded transition-all duration-200 ${
                          activeSection === section.id
                            ...
                            : "text-[#1A1A1A]/45 hover:text-[#1A1A1A]/75 border-l-2 border-transparent"
                        }`}
                      >
            ...
            {/* Article body - first version style */}
            <article className="flex-1 max-w-[680px] pb-32 article-body article-v1">
              {/* Section 1 */}
              <AnimatedSection>
              ...
              <AnimatedSection>
                ...
                <V1CaseCard title="让 AI 成为提问伙伴，而不是代写工具">
              ...
              <AnimatedSection>
                ...
                <V1DefinitionCard title="GitHub 与开源">
              ...
              <AnimatedSection>
                ...
                <V1DefinitionCard title="Harness Engineering">
                ...
                <V1CaseCard title="教师版 Harness：一次安全的 AI 备课流程">
              ...
              <AnimatedSection>
                ...
                <V1DefinitionCard title="API、MCP、约束与循环">
              ...
              <AnimatedSection>
                <div className="pt-24 pb-8">
                  <div className="py-16 text-center border-t border-b border-[#1E40AF]/15">
                    <p className="font-serif text-2xl md:text-4xl leading-[1.5] text-[#1A1A1A] font-bold tracking-tight">
                  ...
                  <div className="mt-20">
                    ...
                    <h3 className="font-serif text-2xl font-bold text-[#1A1A1A] mb-8">面向小学教师的五句使用原则</h3>
                    <div className="space-y-5">
                      {[
                        ...
                        <div key={i} className="flex gap-5 items-start group">
                          ...
                          <p className="text-[1.05rem] leading-relaxed text-[#1A1A1A]/80 pt-1">{principle}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="mt-16 py-10 px-8 bg-[#1E40AF]/[0.04] rounded-xl border border-[#1E40AF]/10 text-center">
                    <p className="font-serif text-xl md:text-2xl font-semibold text-[#1A1A1A] leading-relaxed">
                      AI 是水，老师是河道的设计者。
                    </p>
                  </div>

                  <div className="mt-20 text-center">
                    <p className="font-serif text-base text-[#1A1A1A]/40 italic leading-relaxed max-w-[480px] mx-auto">
              ...
              {/* Footer */}
              <footer className="mt-24 pt-10 border-t border-[#1A1A1A]/8">
                <div className="flex items-center gap-4 mb-6">
                  <img src="/manus-storage/logo-water-flow_29ae1617.png" alt="人工智能如水" className="w-8 h-8 opacity-70" />
                  <div>
                    <div className="font-serif text-sm font-semibold text-[#1A1A1A]/70">人工智能如水</div>
                    <div className="text-xs text-[#1A1A1A]/35">面向小学教师与大众的 AI 智能体认知开场稿</div>
                  </div>
                </div>
                <div className="text-xs text-[#1A1A1A]/25">
                  2026.07 · 保留文字张力版
                </div>
              </footer>
            </article>
        ...
        {/* Mobile TOC button - first version style */}
        <div className={`lg:hidden fixed bottom-6 right-6 z-40 transition-all duration-300 ${showNav ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
          ...
          {mobileNavOpen && (
            <>
              <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-40" onClick={() => setMobileNavOpen(false)} />
              <div className="fixed bottom-20 right-6 z-50 bg-white border border-[#E2E8F0] rounded-xl shadow-2xl p-5 w-64 max-h-[60vh] overflow-y-auto">
                <div className="text-xs font-bold text-[#1E40AF] uppercase tracking-[0.15em] mb-4">目录</div>
                <ul className="space-y-0.5">
                  {sections.map((section) => (
                    <li key={section.id}>
                      <button
                        onClick={() => { scrollToSection(section.id); setMobileNavOpen(false); }}
                        className={`text-left w-full text-[13px] py-2 px-3 rounded transition-all ${
                          activeSection === section.id
  ...
  // ========== NORMAL MODES (OpenAI / Decorated / Editorial) RENDER ==========
  return (
    <div className={`min-h-screen transition-colors duration-300 bg-white`}>
      ...
      <header className={`relative ${isEditorial ? 'pt-20 pb-16 md:pt-32 md:pb-24' : 'pt-16 pb-12 md:pt-24 md:pb-16'}`}>
        ...
        <div className={`relative mx-auto px-6 ${isEditorial ? 'max-w-[780px] text-center' : 'max-w-[680px] text-center'}`}>
          <motion.div
        ...
        <div className={`flex items-center justify-between py-4 border-t border-b ${isEditorial ? 'border-[#1E40AF]/20' : 'border-[#e5e5e5]'}`}>
          <div className="flex items-center gap-1 flex-wrap">
            ...
            <button
              ...
              className={`px-3 py-1.5 text-sm rounded-md transition-all duration-200 ${
                fontSerif
                  ? isEditorial ? 'bg-[#1E40AF]/10 text-[#1E40AF] font-semibold' : 'bg-[#f0f0f0] text-[#0d0d0d] font-semibold'
                  : 'text-[#999] hover:text-[#666]'
              }`}
            >
            ...
            <button
              ...
              className="px-3 py-1.5 text-sm rounded-md transition-all duration-200 text-[#999] hover:text-[#666]"
            >
              <span className="mr-1">□</span>大屏
            </button>
          </div>
          <button
      ...
      <div className={`mx-auto px-6 ${isEditorial ? 'max-w-[1120px]' : 'max-w-[1080px]'}`}>
        <div className="flex gap-0 lg:gap-16 relative">
          ...
          <article className={`flex-1 pb-32 article-body ${isEditorial ? 'max-w-[680px] article-editorial' : 'max-w-[680px]'} ${useSerif ? 'article-serif' : ''}`}>
            {/* Section 1 */}
            <AnimatedSection>
            ...
            {/* Section 2 */}
            <AnimatedSection>
              <h2 id="agent" className="article-h2">从聊天框到智能体：2025 年末的拐点</h2>
              <p>首先，我们来聊聊 AI 智能体。它是理解这一轮变化的核心关键点。</p>
              <p>上一版我把"2025 年末代码类智能体迎来关键拐点、质的飞跃"处理得比较保守。现在看，这个判断应该保留，只是需要讲清楚：所谓"质的飞跃"，不是说 AI 突然变成了真正的人，也不是说它从此永远正确；而是说，代码类智能体开始从"帮你写几段代码"，走向<strong>"在一个环境里持续做事"</strong>。</p>
              <p>这个变化非常关键。过去，我们习惯把人工智能理解成一个聊天框：你问一句，它答一句；你给一个问题，它给一个答案。那是旧阶段的 AI，像一口井，站在旁边打水就可以。</p>
              <p><strong>但智能体的出现，意味着水开始流动。</strong></p>
            ...
            {/* Section 3 */}
            <AnimatedSection>
              <h2 id="generalize" className="article-h2">模型能力为何会泛化</h2>
              <p>很多老师可能会问：代码智能体的突破，和小学教育有什么关系？</p>
              <p>表面看，关系不大。老师不需要每天修复软件漏洞，也不需要提交 GitHub 代码。但如果我们只看"代码"这个外壳，就会错过真正的水势。</p>
              <p>代码任务本质上是一类高度严格的复杂任务。它要求 AI 读懂大量上下文，定位问题，制定步骤，修改文件，运行测试，发现失败，再修正。它不能只写得漂亮，还要经得起执行。换句话说，代码场景逼迫 AI 学会了一个更普遍的能力：<strong>把想法变成可检查的行动</strong>。</p>
              <p>这种能力一旦泛化到教育场景，就会出现完全不同的工作方式。</p>
            ...
            {/* Section 4 */}
            <AnimatedSection>
              <h2 id="teacher" className="article-h2">它如何进入小学教师的日常</h2>
              <p>以一位小学科学老师为例。她要准备《水的三态》这一课。过去，她要自己查资料、设计实验、制作课件、准备提问，还要兼顾学生安全和课堂节奏。现在，她可以对 AI 说：</p>
            ...
            {/* Section 5 */}
            <AnimatedSection>
              <h2 id="barrier" className="article-h2">技术门槛正在被水流冲开</h2>
              <p>就拿 GitHub 上的开源项目与讨论工单来说。如果你几乎没有计算机技术基础，过去根本体会不到开源的魅力。打开 GitHub 页面，你会像被抛到一座陌生码头：满眼都是代码、分支、Issue、Pull Request、README，手足无措，无从下手。</p>
            ...
            {/* Section 6 */}
            <AnimatedSection>
              <h2 id="harness" className="article-h2">Harness Engineering：为水流修建河道</h2>
              <p>说到这里，就必须补充一个正在变得越来越重要的概念：Harness Engineering。</p>
              <p>这个词可以暂时译作"智能体牵引工程""智能体运行框架工程"，也可以用我们这篇文章的比喻来理解：<strong>河道工程</strong>。</p>
            ...
            {/* Section 7 */}
            <AnimatedSection>
              <h2 id="container" className="article-h2">{"\u4ece\u201c\u5199\u63d0\u793a\u8bcd\u201d\u5230\u201c\u8bbe\u8ba1\u5bb9\u5668\u201d"}</h2>
              <p>我们将人工智能比作液体与流水，是有内在缘由的。在智能体领域，我们越来越少地只给它一两条固定有序的提示词。很多时候，我们要依靠目标、语境、偏好、材料、规则和运行框架共同引导它。</p>
              <p>这就好比为水流打造容器：我们搭建外部框架，让内部的力量奔涌而出，把实实在在的成果具象化。这正是我们驾驭水流的方式。</p>
            ...
            {/* Section 8 */}
            <AnimatedSection>
              <h2 id="river" className="article-h2">上善若水，但水需要河道</h2>
              <p>流水本身并不是我们最终想要的东西。很多时候，水只是一种手段。我们搭建好结构框架，水流就会裹挟泥沙杂质分离出去，最终留下清澈规整的成果。另一些时候，我们需要调配溶液。水充当载体，我们就能混合各类原料，调配出各式各样的饮品。</p>
              <p>放到大模型与智能体上也是同理。我们时而需要多样化的选择，时而需要清晰严谨的框架。智能体兼具这两种特质，就如同一柄双刃剑。它能给我们灵感，也会带来误差；它能帮助我们提高效率，也可能让人产生依赖；它能拓展课堂，也可能削弱学生真正的思考。</p>
              <p>所以，AI 教育不是教学生偷懒，而是教学生如何提问。不是教老师交出判断，而是帮助老师把判断变得更清醒。不是让课堂失去人味，而是让老师从重复劳动里抽身出来，把更多时间还给真正的人。</p>
              <p>中国有一句古语：<strong>上善若水</strong>。</p>
            ...
            <AnimatedSection>
              <div className="mt-16 pt-12 border-t border-[#e5e5e5]">
                <h3 className="text-lg font-bold text-[#0d0d0d] mb-6">面向小学教师的五句使用原则</h3>
                <div className="space-y-4 mb-12">
                  {[
                    ...
                    <div key={i} className="flex gap-4 items-start">
                      <span className="shrink-0 text-sm font-bold text-[#999] mt-0.5 w-5">{i + 1}.</span>
                      <p className="text-[15px] leading-[1.75] text-[#374151]">{principle}</p>
                    </div>
                  ))}
                </div>

                <div className="py-10 text-center">
                  <p className="text-xl md:text-2xl font-bold text-[#0d0d0d] leading-[1.6]">
                    最终，带着学生一起驶向那个更开阔、<br className="hidden md:block" />更丰沛，也更值得期待的春天。
                  </p>
                </div>
      ...
      <div className="fixed bottom-6 right-6 z-40 lg:hidden">
        ...
        {mobileNavOpen && (
          <>
            <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-40" onClick={() => setMobileNavOpen(false)} />
            <div className="fixed bottom-20 right-6 z-50 bg-white border border-[#e5e5e5] rounded-xl shadow-2xl p-5 w-60 max-h-[60vh] overflow-y-auto">
              <div className="text-sm font-semibold text-[#0d0d0d] mb-4">目录</div>
              <ul className="space-y-0">
                {sections.map((section) => (
                  <li key={section.id}>
                    <button
                      onClick={() => { scrollToSection(section.id); setMobileNavOpen(false); }}
                      className={`text-left w-full text-[13px] py-2.5 px-3 rounded-md transition-all ${
                        activeSection === section.id
                          ? "text-[#0d0d0d] font-semibold bg-[#f0f0f0]"
